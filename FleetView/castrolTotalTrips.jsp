...
<%!
	...
	Statement stcode,st,st1,st2,st3,st4,st5,st6,st7,st8,stt,st9,st10,st11,sttt,stbrief,stplace,stdebrief,st12,sten,stst,stbt,sten1,sten2,sten3,stst1,sts2,sts3,stbt1,stbt2,stbt3,zst;
	...
	String  exportFileName="CastrolMonthlyTotalTrips.xls";
%>
...
<tr align="left"><td width="150px"><!-- <font color="block" size="3" >Report No:10.6</font>--></td>
<td width="250px">
<%
if(!(null==forchckbox))
{
...
<td width="400px">
<B>Trip categorization By  : </b>
<select name="sortby" id="sortby"  style="">
		   <option value="Select">Select</option>

<%
String sortbyR=request.getParameter("sortby");
System.out.println("value is " +sortbyR);

if(!(null==sortbyR))
		{
	if(sortbyR.equalsIgnoreCase("trans"))
	{
		%>
	...
	else if(sortbyR.equalsIgnoreCase("driver"))
	{
		%>
		
		<option value="trans">Transporterwise</option>
	<option value="driver" selected="selected">Driverwise</option>
	<option value="location" >Start Locationwise</option>
	<option value="totaltrips" >Total Trips</option>
		<%
	}
	else if(sortbyR.equalsIgnoreCase("location"))
	{
		%>
		
		<option value="trans">Transporterwise</option>
	<option value="driver">Driverwise</option>
	<option value="location" selected="selected">Start Locationwise</option>
	<option value="totaltrips" >Total Trips</option>
		<%
	}
	else if(sortbyR.equalsIgnoreCase("totaltrips"))
	{
		%>
...
try
{
	...
	if(!(null==sortby))
	{
		...
		else if(sortby.equals("trans"))
		{
			...
			{
				%>
				<tr align="center"><td><b><font size="2">Castrol Trip Captured For<%=rng%> -By Contractor</font></b></td></tr>
...
{ 
	...
	if(thename.equalsIgnoreCase("ended"))
	{
		...
		<td  class="hed"> Rating</td>
			...
			while(rstt.next())
			{
				...
				vehcountall=vehcountall+rstt.getInt("Vehid");
						...
						String sql14="Select count(Debriefing) as udbcount1 from t_CastrolTripsEnded where Debriefing='Yes' and TripEndDate between '"+fromdate+" 00:00:00' and '"+todate+" 23:59:59'  and StartPlace='"+startplace+"'";
		...
		//	String sqltrip="SELECT Distinct(`TripID`) FROM t_alljddata WHERE `StartDate` between '"+fromdate+"' and '"+todate+"' and Status <> 'NotProcessed'";
			...
			while(rstt.next())
			{
				...
				startplace=rstt.getString("Transporter");
				 String jcode="";
					...
					String str1="select Jcode from t_transporter where TypeValue='"+startplace+"'";
					// System.out.println("\n\nsql-->>"+str1);
					
					  ResultSet rsc1=stcode.executeQuery(str1);
					//System.out.println("\n\njcode-->>"+jcode);
					if(rsc1.next())
					{
						jcode=rsc1.getString("Jcode");
						// System.out.println("\n\njcode-->>"+jcode);
					}
			...
			<tr>
			<td><%=k%></td>
			<td><div align="left"><a href="#" onclick="javascript:window.open('driverwiseTrips_castrol.jsp?transporter=<%=startplace%>&data1=<%=fromdate%>&data2=<%=todate%>&triptype=<%=thename%>');"><%=startplace%></a>
			</div></td>
...
///************************************************STARTED**********************************************************************************************	
	else if(thename.equalsIgnoreCase("started"))
	{
			...
			if(sortby.equalsIgnoreCase("driver" ))
			{
				...
				<td  class="hed"> Rating</td>
					...
					if(select1.equals("primcas") && ((null==select2) || select2.equalsIgnoreCase("null")) && select3.equals("seccas") && ((null==select4) || select4.equalsIgnoreCase("null")) && select5.equals("tancas") && ((null==select6) || select6.equalsIgnoreCase("null")))
					{
						...
						sqll="select  p.DriverName as DriverName,p.DriverID as DriverID,p.Transporter as Transporter,q.TripID as TripID,q.Vehid as Vehid, p.KmTraveled as KmTraveled,"+
		...
		<td  class="hed"> Km. Travelled</td>
		<td  class="hed"> OS Duration<br>(Sec)</br></td>
		<!-- <td  class="hed"> OS Duration<br>Red Zone</td>
		<td  class="hed"> OS Duration<br>Yellow Zone</td>
		<td  class="hed"> OS Duration<br>>65</td>-->
		<td  class="hed"> RA</td>
		<td  class="hed"> RD</td>
		<td  class="hed"> OS Count</td><!--
		<td  class="hed"> OS Count <br>Red Zone</td>
		<td  class="hed"> OS Count<br>Yellow Zone</td>
		<td  class="hed"> OS Count<br>>65</td>-->
		<td  class="hed"> CD</td>
		<td  class="hed"> ND</td>
		<td  class="hed"> Discon.</td>
		<td  class="hed"> DisconnKM.</td>
		...
		//	String sqltrip="SELECT Distinct(`TripID`) FROM t_alljddata WHERE `StartDate` between '"+fromdate+"' and '"+todate+"' and Status <> 'NotProcessed'";
			...
			System.out.println("in started trans");
						...
						if(select1.equals("primcas") && ((null==select2) || select2.equalsIgnoreCase("null")) && select3.equals("seccas") && ((null==select4) || select4.equalsIgnoreCase("null")) && select5.equals("tancas") && (!(null==select6)))
					{
						 //out.println("in 2");
						sqll="select  p.Transporter as Transporter,q.TripID as TripID,q.DriverID as DriverID,q.Vehid as Vehid, p.KmTraveled as KmTraveled,"+
						"p.OSCount as OSCount,p.osInRedZoneCount as osInRedZoneCount,p.osInYellowZoneCount as osInYellowZoneCount,p.osInGreenZoneCount as osInGreenZoneCount"+
						",p.OSDuration as OSDuration,p.osDurationInRedZone as osDurationInRedZone,p.osDurationInYellowZone as osDurationInYellowZone,p.osDurationInGreenZone as osDurationInGreenZone"+
						",p.RACount as RACount,p.RDCount as RDCount,p.DisconnectionCount as DisconnectionCount,p.DisconnectionKM as DisconnectionKM,p.CD as CD,p.ND as ND,q.Briefing as Briefing,"+
						"q.Debriefing  as Debriefing from "+
						"(select Transporter,sum(KmTraveled) as KmTraveled,"+
						"sum(OSCount) as OSCount,sum(osInRedZoneCount) as osInRedZoneCount,sum(osInYellowZoneCount) as osInYellowZoneCount,sum(osInGreenZoneCount) as osInGreenZoneCount"+
						",sum(OSDuration) as OSDuration,sum(osDurationInRedZone) as osDurationInRedZone,sum(osDurationInYellowZone) as osDurationInYellowZone,"+
						"sum(osDurationInGreenZone) as osDurationInGreenZone"+
						",sum(RACount) as RACount,"+
						"sum(RDCount) as RDCount,sum(DisconnectionCount) as DisconnectionCount,sum(DisconnectionKM)as DisconnectionKM,sum(CD) as CD,sum(ND) as ND "+
						"from "+ 
						"(SELECT Transporter, sum( KmTraveled ) AS KmTraveled,sum( OSCount ) AS OSCount"+
						",sum(osInRedZoneCount) as osInRedZoneCount,sum(osInYellowZoneCount) as osInYellowZoneCount,sum(osInGreenZoneCount) as osInGreenZoneCount,"+
						" sum( OSDusration ) AS OSDuration,sum(osDurationInRedZone) as osDurationInRedZone,sum(osDurationInYellowZone) as osDurationInYellowZone,"+
						" sum(osDurationInGreenZone) as osDurationInGreenZone, "+
						"sum( RACount ) AS RACount, sum( RDCount ) AS RDCount,sum(DisconnectionCount) as DisconnectionCount,sum(DisconnectionKM)as DisconnectionKM,sum(CD) as CD,sum(ND) as ND "+
						"FROM t_CastrolTripsStarted WHERE TripStartDate BETWEEN '"+fromdate+" 00:00:00' and '"+todate+" 23:59:59' "+
						" AND Category IN ('primary','secondary') GROUP BY Transporter "+
							"UNION ALL "+
						"SELECT Transporter, sum( Distance ) AS KmTraveled, sum( OSCount ) AS OSCount,"+
						"sum(osInRedZoneCount) as osInRedZoneCount,sum(osInYellowZoneCount) as osInYellowZoneCount,sum(osInGreenZoneCount) as osInGreenZoneCount,"+
						" sum( OSDuration ) AS OSDuration,sum(osDurationInRedZone) as osDurationInRedZone,sum(osDurationInYellowZone) as osDurationInYellowZone, "+
						"sum(osDurationInGreenZone) as osDurationInGreenZone, sum( RACount ) AS RACount, sum( RDCount ) AS RDCount,sum(DisconnectionCount) as DisconnectionCount,sum(DisconnectionKM)as DisconnectionKM,sum(CD) as CD,sum(ND) as ND FROM t_CastrolStartedDriverkm "+ 
...
//  						out.println(sqll);
					...
					int osCountRed=0,osCountYellow=0,osCountGreen=0;
					int osDurationRed=0,osDurationYellow=0,osDurationGreen=0;
					int TotalOsCountRed=0,TotalOsCountYellow=0,TotalOsCountGreen=0;
					int TotalOsDurationRed=0,TotalOsDurationYellow=0,TotalOsDurationGreen=0;
			...
			while(rstt.next())
			{
				...
				vehcountall=vehcountall+rstt.getInt("Vehid");
						...
						/*
						  ...
						  osInRedZoneCount, osInYellowZoneCount,osInGreenZoneCount"+
						  osDurationInRedZone,osDurationInYellowZone,osDurationInGreenZone"+
						
						*/
						osCountRed=rstt.getInt("osInRedZoneCount");
						TotalOsCountRed+=osCountRed;
						osCountYellow=rstt.getInt("osInYellowZoneCount");
						TotalOsCountYellow+=osCountYellow;
						osCountGreen=rstt.getInt("osInGreenZoneCount");
						TotalOsCountGreen+=osCountGreen;
						
						osDurationRed=rstt.getInt("osDurationInRedZone");
						TotalOsDurationRed+=osDurationRed;
						osDurationYellow=rstt.getInt("osDurationInYellowZone");
						TotalOsDurationYellow+=osDurationYellow;
						osDurationGreen=rstt.getInt("osDurationInGreenZone");
						TotalOsDurationGreen+=osDurationGreen;
		 ...
		 -->	
		  ...
		  <td>
			...
			%>
						...
						<!-- <td><div align="right">
			< %
				out.print(osDurationRed);
			%>
			</div></td>
						<td><div align="right">
			< %
				out.print(osDurationYellow);
			%>
			</div></td>
						<td><div align="right">
			< %
				out.print(osDurationGreen);
			%>
			</div></td>-->
			<td><div align="right">
			<%
			...
			< %
			out.print(osCountRed);
				
			%>
			</div></td>		<td><div align="right">
			< %
			out.print(osCountYellow);
				
			%>
			</div></td>		<td><div align="right">
			< %
			out.print(osCountGreen);
			...
			<!--<td class="hed"><div align="right">< %=TotalOsDurationRed%></div></td>
			<td class="hed"><div align="right">< %=TotalOsDurationYellow%></div></td>
			<td class="hed"><div align="right">< %=TotalOsDurationGreen%></div></td>-->
			
			<td class="hed"><div align="right"><%=racountall%></div></td>
			<td class="hed"><div align="right"><%=rdcountall%></div></td>
			<td class="hed"><div align="right"><%=oscountall%></div></td><!--
			<td class="hed"><div align="right">< %=TotalOsCountRed%></div></td>
			<td class="hed"><div align="right">< %=TotalOsCountYellow%></div></td>
			<td class="hed"><div align="right">< %=TotalOsCountGreen%></div></td>--> 
			<td class="hed"><div align="right"><%=cdcountall%></div></td>
			<td class="hed"><div align="right"><%=ndcountall%></div></td>
			<td class="hed"><div align="right"><%=disconnectionCountAll%></div></td>
		<td class="hed"><div align="right"><%=disconnectionKMAll%></div></td>
