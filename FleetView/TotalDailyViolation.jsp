...
<%!
	...
	String fromdate, todate,drivernm,driverid,driverlist;
...
try
{
	...
	//String Transporter=session.getAttribute("usertypevalue").toString();
			...
			if(thename.equals("started"))
			{
				...
				<tr align="center"><td><b><font size="3">Daily Driving Hrs Violation Report from <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(fromdate)) %> To <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(todate)) %></font></b></td></tr>
				 <tr><td><div class="bodyText" align="left"> 
	 ...
	 <a href="excel_totalDailyViolation.jsp?fromdate=<%=fromdate%>&todate=<%=todate%>&thename=<%="started"%>&driverlist=<%=list %>"  title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a> 
	 &nbsp;&nbsp;&nbsp;<a href="#" onclick="javascript:window.open('print_totalDailyViolation.jsp?fromdate=<%=fromdate%>&todate=<%=todate%>&thename=<%="started"%>&driverlist=<%=list %>');" title="Print Current Position Report"><img src="images/print.jpg" width="15px" height="15px"></a>
...
///************************************************STARTED**********************************************************************************************	
	else if(thename.equalsIgnoreCase("started"))
	{
		...
		<%
				...
				//String PSvehlist="(";
				String sql="select * from t_CastrolDrivers6 where TheDte between '"+fromdate+"' and '"+todate+"' AND DrinHrs > 10 and driverid IN "+list+" order by TheDte Asc";
