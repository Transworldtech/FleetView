...
<!-- JavaScript for export to excel and export to PDF -->
	...
	{  
	 ...
	 function birt(){
			...
			alert("Formated Date1 "+formattedDate1);
			alert("Formated Date1 "+formattedDate2);
			...
			document.getElementById("myIframe").src="http://myfleetview.com:8080/birttest/frameset?__report=fuelinfo.rptdesign&usertypevalue="+user+"&vehnum="+category+"&Date1="+formattedDate1+"&Date2="+formattedDate2+"&dt="+date1+"&dt1="+date2+"&__showtitle=false";
	...
	%>
	 <table border="0" width="600" align="center" class="stats">
		<tr>
			...
			<font face="Arial" color="black" size="3"><b> Fuel Report </b></font></td>
		</tr>
	</table> 
...
&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<select name="fuel" id="fuel"> 
 		   ...
 		   <% 
 		  	String sql = "select Distinct(VehNo) from db_gps.t_fuelinfo order by updatedDtTime desc;";
			ResultSet rs = st.executeQuery(sql);
			while(rs.next())
			{
				%>
				<option value="<%=rs.getString("VehNo") %>" ><%=rs.getString("VehNo") %></option>
				<%
			}
			%>
 		     </select> 
...
<div id="reportData1">
<iframe id="myIframe" width="100%" height="850px" src="http://myfleetview.com:8080/birttest/frameset?__report=fuelinfo.rptdesign&usertypevalue=<%=User%>&vehnum=All&Date1=<%=dt%>&Date2=<%=dt1%>&dt=<%=datex1%>&dt1=<%=datex2%>&__showtitle=false"> 
