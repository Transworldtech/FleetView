...
function passData(id)
{
	...
	var deviationType=  document.getElementById("deviationType").value;
...
//	alert(transporter);
	var radList = document.getElementsByName("strtpl");
	//alert(radList);
	//alert(document.getElementById("strtpl"+id).checked);
	if(document.getElementById("strtpl"+id).checked==true)
	{
		var rad=document.getElementById("strtpl"+id);
		var data=rad.value;
	//	alert(data);
		sp=data.split("#");
		popitup("routeZoneFeedbackForm.jsp?tripid="+tripid+"&driverid="+driverid+"&transporter="+transporter+"&vehid="+vehid+"&lat="+sp[0]+"&lon="+sp[1]+"&startplace="+sp[2]+"&deviationType="+deviationType);
	}
//	popitup("routeZoneFeedbackForm.jsp?tripid="+tripid+"&driverid="+driverid+"&transporter="+transporter+"&vehid="+vehid+"&startplace"+chck);
}



function popitup(url) {
	newwindow=window.open(url,'name','height=225,width=500,top=100,left=400,resizeable=0,scrollbars=0');
...
<%!
String VehicleCode, fromdate, todate, desc,vehregno,fromtime,totime,ftime1,ftime2,ttime1,ttime2,transporter,driverid,tripid;
...
if("bd".equalsIgnoreCase(fromPage)){
...
if(!(null==request.getParameter("data")) || "bd".equalsIgnoreCase(fromPage))
{
/* all code comes here */
%>
...
%>
	<tr>
		<td><input type="radio" id="strtpl<%=i%>" name ="strtpl" value="<%=rst1.getString("LatinDec")%>#<%=rst1.getString("LonginDec")%>#<%=rst1.getString("TheFieldSubject") %>" onclick="passData(<%=i%>)"></input></td>
