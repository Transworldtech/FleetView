...
<tr><th class="hed">Rapid Acceleration Report For <%=hvrno %> From <%=session.getAttribute("HisShowDate").toString()%> <%=fff%> to <%=session.getAttribute("HisShowDate1").toString()%> <%=ttt%>
</th></tr>
...
//String sql="SELECT * FROM t_veh"+rvehid+" where concat(TheFieldDataDate,TheFieldDataTime)>='"+ date1+fff+"' and  concat(TheFieldDataDate,TheFieldDataTime)<='"+ date2+ttt+"' and TheFiledTextFileName='AC' order by TheFieldDataDate desc,TheFieldDataTime desc";
String sql="SELECT * FROM t_veh"+rvehid+"_ra where concat(TheDate,' ',TheTime)>='"+ date1+" "+fff+"' and  concat(TheDate,' ',TheTime)<='"+ date2+" "+ttt+"' order by TheDate desc,TheTime desc";
//System.out.println("HRA Count::>>"+sql);
ResultSet rst1=st1.executeQuery(sql);
while(rst1.next())
{
if (rst1.getInt("FromSpeed")>5)
	{
	%>
...
%></td>
	...
	<td class="bodyText" style="text-align: left"><div align="left"><%=fleetview.ShowOnMapOnlyByDateTime(rvehid,rst1.getString("TheDate"),rst1.getString("TheTime")) %></div></td>
</tr>
