...
<%
		try {
			...
			for (int counter = 0; counter < numRecords; counter++) 
			{
				...
				if (currentRecord.getLocationWithDeviceStatus().contains("Device Disconnected")) {
	%>	
		<tr>
			...
			<td id="tdd<%=i%>" style="<%=currentRecord.getBgColor()%>">
				...
				<div class="popup" id="popupx<%=i%>">
					<table border="1" bgcolor="white">
		   				<tr>  
						<td> 
							<a href="ShowCustomReportonTWMap.jsp?tripid=<%=currentRecord.getTripID()%>
										...
										&vehregno=<%=currentRecord.getVehicleRegistrationNo()%>
										&ownername=<%=currentRecord.getTransporter()%>
										&startplace=<%=currentRecord.getTripOrigin()%>
										&endplace=<%=currentRecord.getTripDestination()%>
										&pg=st" onclick="toggleDetails1(<%=i%>,false);">Show Trip on Map </a>
						</td>
		   				</tr>
<%
