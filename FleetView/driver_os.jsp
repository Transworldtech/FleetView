...
{  
          ...
          document.OS.action ="excel.jsp";
          document.forms["OS"].submit();
} 

</script>
<%
String exportFileName=session.getAttribute("user").toString()+"_OSExcepReport.xls";
%>
<form id="OS" name="OS"  method="post" >
<div id="table1">
...
<font size="3">Over Speed Details for Driver <%=dname%> ( <%=dcode%> ), Vehicle <%=vehreg%> of Transporter <%=transporter%> for <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(data1))%>.</font>
...
try{
	...
	sql="select * from t_driver_overspeed where Transporter='"+transporter+"' and FromDate='"+data1+"' and dcode='"+dcode+"' order by concat(FromDate,FromTime) asc";
	...
	<td align="right" style="width: 30px;"><div align="right"><%=new SimpleDateFormat("HH:mm").format(new SimpleDateFormat("HH:mm:ss").parse(rst.getString("FromTime")))  %></div></td>
	<td align="right" style="width: 25px;"><div align="right"><%=rst.getString("Speed")%></div></td>
