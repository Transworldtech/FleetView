...
if(limitcnt==0)
{
	sql = "select VehicleRegNumber  from db_gps.t_vehicledetails where Replace(Replace( Replace( Replace( VehicleRegNumber ,'.', '') ,' ','') ,'-','') ,'_','')  like '%"+vehicle+"%' and vehiclecode in "+vehlist+" order by VehicleRegNumber";
}
else
{
   sql = "select VehicleRegNumber  from db_gps.t_vehicledetails where Replace(Replace( Replace( Replace( VehicleRegNumber ,'.', '') ,' ','') ,'-','') ,'_','')  like '%"+vehicle+"%' and vehiclecode in "+vehlist+" order by VehicleRegNumber limit 0,"+limitcnt+"";
}
