...
%>
			...
			<div  align="center"><font size="3"><b>The Overspeed Report for <%=fleetview.getVehRegNo(vehcode) %> On <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(fromdate)) %></b></font></div>
				<div align="right">
				<a href="#" onclick="javascript:window.open('printnewos.jsp?vehcode=<%=vehcode %>&fromdate=<%=fromdate %>');" title="Print Report"><img src="images/print.jpg" width="15px" height="15px"></a>&nbsp; &nbsp; 
				<a href="excelnewos.jsp?vehcode=<%=vehcode %>&fromdate=<%=fromdate %>"  title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a>
				<%=fleetview.PrintDate()%>
				</div>
			</td>
			</tr>
			...
			<%
			String sql="select * from t_veh"+vehcode+" where TheFieldDataDate='"+fromdate+"' and TheFiledTextFileName='OS'";
