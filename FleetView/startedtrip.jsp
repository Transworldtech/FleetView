...
String sql,tripid,vehcode,sdate,Stamp,label1,sql1,sql2,osstr,acstr,dcstr,ststr,pg,colorcode,nstd,nspd;
String lat, lon,location,thedate,thetime,lat1,lon1,vehregno,ownername,lastlocation,startplace,endplace,enddate,sdate1,enddate1,stopdatetime, startdatetime;;
String thedate1="",thetime1="",latStart="",longStart="",locationStart="",thedateStart="",thetimeStart="",firstlocation="-",location1="-";
...
sdate=request.getParameter("sdate");//startdatetime
enddate=request.getParameter("enddate");//enddatetime
...
//String usern=session.getAttribute("usertypevaluemain");
   ...
   if (GBrowserIsCompatible()) 
	{
       ...
       arrowIcon.infoWindowAnchor = new GPoint(0,0);
		...
		map.addControl(new GScaleControl());
     	...
     	{
     	%>
		var gx = new GGeoXml("http://myfleetview.com/FleetView/KML/Routefrom_Mumbai_to_Delhi.kml");
		map.addOverlay(gx);
		<%
     	}
     	%>
...
try{
	...
	String sql11="select * from t_veh"+ vehcode +" where concat(TheFieldDataDate,' ',TheFieldDataTime) >='"+sdate+"' and concat(TheFieldDataDate,' ',TheFieldDataTime) <='"+enddate+"' and TheFiledTextFileName IN('SI','OF') order by concat(TheFieldDataDate,TheFieldDataTime) desc";
	    
	ResultSet rst22=st2.executeQuery(sql11);
	if(rst22.next())
	{
        lat = rst22.getString("LatinDec"); 
		lon = rst22.getString("LonginDec"); 
		location= rst22.getString("TheFieldSubject");
		thedate= rst22.getString("TheFieldDataDate");
		thetime = rst22.getString("TheFieldDataTime"); 
		dist=rst22.getInt("Distance");
...
String sql12="select * from t_veh"+ vehcode +" where concat(TheFieldDataDate,' ',TheFieldDataTime) >='"+sdate+"' and concat(TheFieldDataDate,' ',TheFieldDataTime) <='"+enddate+"' and TheFiledTextFileName IN('SI','OF') order by concat(TheFieldDataDate,TheFieldDataTime) asc";
//System.out.println("sql1====>"+sql1);
ResultSet rst12=st.executeQuery(sql12);
if(rst12.next()){
	lat=rst12.getString("LatinDec");
	lon=rst12.getString("LonginDec");
	location=rst12.getString("TheFieldSubject");
	thedateStart=rst12.getString("TheFieldDataDate");
	thetimeStart=rst12.getString("TheFieldDataTime");
	firstlocation="Start Location :"+thedateStart+" "+thetimeStart+" "+location;
...
/******************code for stops***************/
	...
	String sql111="select * from t_veh"+vehcode+" where TheFieldDataDate >='"+sdate+"' and concat(TheFieldDataDate,' ',TheFieldDataTime) <='"+enddate+"' and TheFiledTextFileName='SI' order by concat(TheFieldDataDate,TheFieldDataTime) asc ";
	System.out.println("StopsSql::>>"+sql);
	ResultSet rst2=st.executeQuery(sql111);
	int stops=Integer.parseInt(session.getAttribute("stop11").toString());
