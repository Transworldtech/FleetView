...
function showExcel()
{
	...
	window.open("exportactivetrips.jsp?&sessiondata=VehiclePositionsViewData&key=ALL&condition="+valex);
...
<%
	...
	final String userTypeValuemain = session.getAttribute("usertypevaluemain").toString();
	//System.out.println(">>>>>>>>>++ " +userTypeValue);
	
	//System.out.println(">>>>>USERTYPEVALUE MAIN>>>>++ " +userTypeValuemain);
...
<%
	try {
		...
		for (int counter = 0; counter < numRecords; counter++) {
			...
			//System.out.println("TRIP ID=@@@@@@@@@@=>"+currentRecord.getTripID());
			
			
			if(!(currentRecord.getTripID().equalsIgnoreCase("-"))  &&  !(currentRecord.getTripID().contains("Enter")) )
			{
				...
				{
					String sqltrp="select gpname from db_gps.t_startedjourney where tripid='"+currentRecord.getTripID()+"'  and gpname='"+userTypeValue+"'  ";
					ResultSet rstNew1=st7.executeQuery(sqltrp);
					System.out.println("sqltrp==>"+sqltrp);
					if(rstNew1.next())
					{
						
						gpname=rstNew1.getString("gpname");	
		...
		%>	
				<tr>
					<td style="text-align:right;<%=currentRecord.getBgColor()%>"><%=p%></td>
					<%
		...
		%>
				   	 <tr>
						<td> <a href="temperaturegraph.jsp?vehcode=<%=currentRecord.getVehicleCode()%>" onclick="toggleDetails(<%=i%>,false);">Temperature Graph </a>
						</td>
				   	</tr>
				   	<%
					...
					try
					{
					   ...
					   ResultSet rstNew=st1.executeQuery(SqlVehType);
					   if(rstNew.next())
					   {
						   vehtype=rstNew.getString("vehtype");
					   }
					   else
					   {
						   vehtype="-";
					...
					if (vehicleDao.isSensorDataDisplayed(userTypeValue,fromPage)) {
						...
						{			
							...
							try
							{
								...
								{
									System.out.println("sensor1 else ");
						...
						{	
							...
							try
							{
								...
								ResultSet rssen1 = st1.executeQuery(sqlsen1);
								System.out.println("sensor2   "+sqlsen1);
								if(rssen1.next())
								{
									System.out.println("sensor2 if ");
								...
								{
									System.out.println("sensor2 else ");
							...
							{
								e.printStackTrace();
								System.out.println("sensor2 catch ");
								sensor = currentRecord.getSensor2();
							}
						...
						try
						{
							...
							if(rssen1.next())
							{
								...
								else
								{
									...
									System.out.println(" sqlsk "+sqlsk);
		...
		<%
					...
					System.out.println("GP NAME==>"+gpname);
