...
{  
          ...
          document.ND.action ="excel.jsp";
          document.forms["ND"].submit();
} 

</script>
<%
String exportFileName=session.getAttribute("user").toString()+"_NDExcepReport.xls";
%>
<form id="ND" name="ND"  method="post" >
<div id="table1">
...
<font size="3">Night Driving Details for Driver <%=dname%> ( <%=dcode%> ), Vehicle <%=vehreg%> of Transporter <%=transporter%> for <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(data1))%></font>
...
try{
	...
	sql="select * from t_driver_nd where Transporter='"+transporter+"' and FromDate='"+data1+"'and dcode='"+dcode+"' order by concat(FromDate,FromTime) asc";
	ResultSet rst=st2.executeQuery(sql);
	System.out.println("aaaaaaaaaaaaaaaaaaa" +sql);
	int i=1;
	while(rst.next())
	{
	%>
	...
	System.out.println("aaaaaaaaaaaaaaaaaaa111" +sql);
	if(rst1.next())
	{
		%>
