...
String filename=session.getAttribute("user").toString()+showdatex+"_Driver_rd_report.xls";
...
<tr>
<td align="center">
<font color="brown" size="2">The Rapid Deceleration details for driver <%=dname%> ( <%=dcode%> ), Vehicle <%=vehreg%> of transporter <%=transporter%> for date <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(data1))%>.</font>
<div align="right">
...
try{
	...
	sql="select * from t_driver_rd where TheDate='"+data1+"' and dcode='"+dcode+"' order by concat(TheDate,TheTime) asc";
