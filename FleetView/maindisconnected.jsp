...
<%!String condtn="ShowTripInfo"; Connection conn=null, conn1=null;
	Statement st1=null;%>
...
function showExcel()
{
	...
	window.open("MainsDiscExcel.jsp?condition="+valex);
...
<table width="100%" align="center">
	<tr>
		...
		<div><font size="3"><b> Mains Disconnected Vehicle
		</b></font></div></td>
...
<%
			...
			%>
			<td id="tdd<%=i%>" style="text-align:left;<%=currentRecord.getBgColor()%>" >
...
%>
						<tr>
						<td>
						<a href="javascript: flase;" onClick="window.open ('endtrip.jsp?tripid=<%=currentRecord.getTripID()%>&vehcode=<%=currentRecord.getVehicleCode()%>&vehno=<%=currentRecord.getVehicleRegistrationNo()%>','win1', 'width=750, height=600, location=0, menubar=0, scrollbars=0, status=0, toolbar=0, resizable=0');javascript:toggleDetails1(<%=i%>,false);">End Trip </a>
						</td>
		   				</tr>
			  			<tr>
						<td> 
...
%>
			...
			<td id="tdf<%=i%>" style="text-align:left;<%=currentRecord.getBgColor()%>;"><%=currentRecord.getFuelLevel()%></td>
<%
	if ("FUELFLOW".equalsIgnoreCase(currentRecord
						.getUnitDescription())) {
%>
			<td id="tdl<%=i%>" style="text-align:left;<%=currentRecord.getBgColor()%>;"><a href="showfuelalert.jsp?vehcode=<%=currentRecord.getVehicleCode()%>&data=<%=getfuelAlert_AddedTodayDate()%>&data1=<%=getfuelAlert_AddedTodayDate()%>"><%=currentRecord.getFuelAlert()%></a></td>
			
<%
				} else {
			%>
			<td id="tdl<%=i%>" style="text-align:left;<%=currentRecord.getBgColor()%>;"><%=currentRecord.getFuelAlert()%></td>
...
<%
			...
			<td style="text-align:right;<%=currentRecord.getBgColor()%>"><%=currentRecord.getSensor4()%></td>			
<%
				}
			%>
		</tr>
