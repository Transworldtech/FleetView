...
<%
Connection con1;
Statement stmt1,stmt2,st,stmt3;
...
String datex1="", datex2="", data1="", data2="",user2="",user3="";
String dateformat="",dataDate1 = "",dataDate2 = "",mentstatus="",vehicles="",chbxAll="",chkbox="";
String enterdate="",veh_new = "",date1_new = "",date2_new = "",status_new = "",VehRegNo="";
%>
...
user3=session.getValue("usertypevalue").toString();
 user2="ETAAlert"+user3;
System.out.println("valueeeeeeee"+user2);
...
<div style="font-size: 1.8em; text-align: center; margin-top: 0.5em; margin-bottom: 0.4em;margin-left: 0.6em" align="center"><font face="Arial" size="3">
	<B>Vehicle Maintenance Report</B></font></div>
		...
		<td>
         <b>Vehicle:</b>
           <select name="vehicle" id="vehicle" style="width: 125px; height:20px; border: 1px solid black; font: normal 11px Arial, Helvetica, sans-serif; color: #000000; border-color: activeborder;">
            ...
            <%
            try{
            String destination="Hydrabad";
            String sql56= "select * from db_gps.t_branchwisemailid  where branch = '"+destination+"' and  ownername LIKE '%" + user3 + "%'"; 
            System.out.println("Sql wali query "+sql56);
            ResultSet rsvideo1=stmt3.executeQuery(sql56);
            while(rsvideo1.next())
            {
...
<a href="#"
					...
					height="15px"></a>&nbsp; &nbsp; <a
					href="VehicleMaintenanceExcel.jsp?data=<%=date1_new%>&data1=<%= date2_new%>&status=<%= status_new%>&vehicle=<%=veh_new%>&VehicleRegNo=<%=vehicles %>&extAll=<%=chbxAll %>&inserted=<%=inserted %>&inserted1=<%=inserted2 %>&inserted3=<%= inserted3%>"
...
<td style="text-align: left"><%=currentRecord1.getTransporter()%>
</td>
<%
...
<td style="text-align: left"><%=currentRecord1.getSrviceName()%>
</td>
<%
...
{
%>
<td style="text-align: left"><%=currentRecord1.getVendorname()%>
</td>
<%
}
%>
...
%>
<td style="text-align: left"><%=currentRecord1.getEmailTo()%></td>
...
<td style="text-align: left"><%=currentRecord1.getEnteredBy()%></td>
<%
}
%>
<%
