...
String filename=session.getAttribute("user").toString()+showdatex+"_driver_nd_report.xls";
...
<tr>
<td align="center">
<font color="brown" size="2">The Night Driving details for driver <%=dname%> ( <%=dcode%> ), Vehicle <%=vehreg%> of transporter <%=transporter%> for date <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(data1))%>.</font>
<div align="right">
...
try{
	...
	sql="select * from t_driver_nd where FromDate='"+data1+"' and dcode='"+dcode+"' order by concat(FromDate,FromTime) asc";
