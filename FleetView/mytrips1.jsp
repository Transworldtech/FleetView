...
sql="select distinct(tripid),Vehid,TripRating,VehRegNo,OwnerName,StartPlace,StartCode,StartDate,StartTime,EndPlace,EndCode,EndDate,EndTime,StartKm,EndKm,KmTravelled,UnloadDateTime,LoadingDelay,UnloadingDelay,JourneyStatus,DriverID,DriverName,JDCode,Weight,Vendor from t_completedjourney where  (OwnerName='"+session.getAttribute("usertypevalue").toString()+"' OR GPName='"+session.getAttribute("usertypevalue").toString()+"')    and StartDate BETWEEN '"+fromdate+" 00:00:00' AND '"+todate+" 23:59:59'  order by StartDate ";
...
KmTravelled=rst.getInt("KmTravelled");

LoadingDelay=rst.getString("LoadingDelay");
if(LoadingDelay==null || LoadingDelay.equalsIgnoreCase("null"))
{
	LoadingDelay="-";
}
UnloadingDelay=rst.getString("UnloadingDelay");
if(UnloadingDelay==null || UnloadingDelay.equalsIgnoreCase("null"))
{
	UnloadingDelay="-";
}
...
<a href="javascript:toggleDetails(<%=i%>,true);" title="Click To See the Reports"><font size="2"><%=rst.getString("TripId")%></font></a> </font><br />
<div class="popup" id="popup<%=i%>">
<table border="1" bgcolor="white">
...
<tr><td>
<a href="#" onclick="javascript:window.open('endtrip1.jsp?tripid=<%=rst.getString("Tripid")%>&EDate=<%=EDate%>&ETime=<%=ETime%>&vehcode=<%=rst.getString("VehId")%>','','menubar=1,resizable=2,width=770,height=350,location=no');">Change Unload Time</font> </a>
<!-- <a href="endtrip1.jsp?tripid=<%=rst.getString("Tripid")%>&EDate=<%=EDate%>&ETime=<%=ETime%>&vehcode=<%=rst.getString("VehId")%>">Change Unload Time</a> -->
</td>
<tr><td><a href="javascript:toggleDetails(<%=i%>,false);">Close</a></td></tr>
</table>
</div> 
...
<td style="text-align: left"><div align="left"><%=rst.getString("DriverName")%> <%="("+rst.getString("DriverId")+")"%></div></td>

<td style="text-align: left"><%=rst.getString("VehRegNo")%></td>

<%
String sql3="Select VehDesc,VehModel,VehType1 from t_vehicledetails where VehicleRegNumber='"+rst.getString("VehRegNo")+"'";
ResultSet rst3=stt2.executeQuery(sql3);
System.out.println("veh desc"+sql3);
if(rst3.next())
{
...
<td id="tdu<%=i%>"style="text-align: left; display: none;"><%
try{ 
	out.print(new SimpleDateFormat("dd-MMM-yyyy HH:mm:ss").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rst.getString("UnloadDateTime"))));
}catch(Exception ee)
{
	out.print("-");
}
...
<td style="text-align: right"><%=nf1.format(rst.getInt("TripRating"))%></td>
