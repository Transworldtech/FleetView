...
String route="", entby="",veh="", stloc="", othstloc="", stdate="", sttimehour="", sttimemin="", rptdate="", rpttimehour="", rpttimemin="",rptdttm="", 
dest="", othdest="", driverid="", othdrivid="", newdrvname="", drivername="", tripid="", stloc1="",vendor="",s="",TripCategory="",customerMailID="",fname="",dest1="",GoodsContent="" ;
String vehcode="", stlocinsrt="",stlocinsrt1="",destinsrt="",destinsrt1="", driveridinsrt="", user="",user1="", sttime="",calender1="",calender="",etadate="",etadate1="", etatime1="", etatime2="", etatime="",othstrtloc="",joborderno="",revenue="",containerno="";
...
String sql7="",weight="",stlocnew="",stlocnew1="",castrolroutecode="",castrolroutecode1="",othvendor="",othTripCategory="",othGoodsContent="",loadreason="",fxdkm="",ftme="",frie="",loadcmnt="",adv="",owner="",allemailid="";
...
/* ***************Checking whether trip id is duplicate or not*********************** */
  sql15="select * from t_startedjourney where TripId='"+tripid+"' ";
  rs15=stmt1.executeQuery(sql15);
  if(rs15.next())
  {
...
else
{
	System.out.println("i-----n--esle----->");
  	...
  	stlocinsrt=stloc1;
  	stloc=stlocinsrt;
	System.out.println("stloc Fixed12345"+stloc);
  	System.out.println("stlocinsrt  " +stlocinsrt);
  	destinsrt=dest1;
  	dest=dest1;
  	System.out.println("dest Fixed12345"+dest);
...
if(route.equals("No"))
{
   if(!(dest.equals("Other")))
   {
     sql9="select * from t_warehousedata where WareHouse='"+destinsrt+"' ";
     rs9=stmt1.executeQuery(sql9);
     if(rs9.next())
     {
        latit=rs9.getFloat("Latitude");
        System.out.println("sqlcode-------****************> "+latit);
        
        longit=rs9.getFloat("Longitude");
        System.out.println("sqlcode-------*****************> "+longit);
     } 
   }
}
else{
	sql9="select * from t_warehousedata where WareHousecode='"+destinsrt1+"' ";
    rs9=stmt1.executeQuery(sql9);
    System.out.println("Warehouse-------****************> "+sql9);
    if(rs9.next())
    {
       latit=rs9.getFloat("Latitude");
       System.out.println("sqlcode-------****************> "+latit);
			...
			if(rsinstall.next()){
				...
				if(rsroute.next()){
					...
					String newrouteid=rs.getString("JRMRouteID");
						...
						try
						{
							 String JRMCheckRoute="insert into db_gps.t_JRMCheckRoute(Vehcode,TripId,VehRegNo,OwnerName,StdCode,EndCode,UpdatedDateTime)values('"+vehcode+"','"+tripid+"','"+veh+"','"+oname+"','"+stlocinsrt1+"','"+destinsrt1+"','"+now1+"')";
 ...
 //end of code for sending instructions
  ...
  try
  {
	  sql2="insert into t_startedjourney (TripId, VehId, VehRegNo, StartPlace, StartCode, EndPlace, EndCode, EndLat, EndLong, ReportDateTime, StartDate, StartTime, DriverCode, DriverName, Ownername, GPName, JStatus, ETA,Weight,Vendor,timetochecktripend,FIxedKm,FIxedTime,TripCategory,GoodsContent,Advance,Frieght,Loadreason,Loadcomments,EntryBy,customerMailID,cnnumber,lrissuedby,lrcontact,branchoffice,cnfilename,rate_hour,rate_km,tripexpence,joborderno,revenue,containerno) values ('"+tripid.replaceAll("'"," ")+"', '"+vehcode+"', '"+veh+"','"+stlocinsrt.replaceAll("'"," ")+"', '"+stlocinsrt1start+"', '"+destinsrt.replaceAll("'"," ")+"', '"+destinsrt1dest+"', '"+latit+"', '"+longit+"','"+rptdttm+"', '"+stdate+"', '"+sttime+"', '"+driveridinsrt.replaceAll("'"," ")+"','"+drivername.replaceAll("'"," ")+"','"+owner.replaceAll("'"," ")+"','"+user.replaceAll("'"," ")+"','Running', '"+etadate+" "+etatime+"','"+weight+"','"+vendor.replaceAll("'"," ")+"','"+s+"','"+fxdkm+"','"+ftme+"','"+TripCategory.replaceAll("'"," ")+"','"+GoodsContent.replaceAll("'"," ")+"','"+adv+"','"+frie+"','"+loadreason.replaceAll("'"," ")+"','"+loadcmnt.replaceAll("'"," ")+"','"+entby+"','"+allemailid+"','"+cnno.replaceAll("'"," ")+"','"+lrissue+"','"+lrcontact+"','"+boffice.replaceAll("'"," ")+"','"+InsertDateTime+"_"+itemName+"','"+rateperhour+"','"+rateperkm+"','"+tripexpence+"','"+joborderno+"','"+revenue+"','"+containerno+"') ";
...
{
 sql7="insert into t_warehousedata(WareHouse,Owner) values('"+othstrtloc+"','"+user+"')";
...
if(!othdest.equals(""))
{
 sql7="insert into t_warehousedata(WareHouse,Owner) values('"+othdest+"','"+user+"')";
 ...
 stmt3.executeUpdate(sql7);
		...
		//String sqlmaster11="select toid, Tocc from db_gps.t_branchwisemailid where ownername='"+user1+"' and branch='"+destinsrt+"'";
		...
		if((Trip.equals("Trip") || Trip == "Trip")  && (Customer.equalsIgnoreCase(user.trim()) || Customer == user.trim()))//&& (Customer.equalsIgnoreCase(user.trim()) || Customer == user.trim())
		{
			System.out.println("In if");
			    ...
			    /* strhtml = strhtml+"<tr bgcolor=^#ADDFFF^><td align=center style=^width: 5%;^><font size=^2^ face=^arial^><b> Sr<br>No </b></font></td><td align=center style=^width: 5%;^><font size=^2^ face=^arial^><b> Trip<br>ID </b></font></td><td align=center style=^width: 25%;^><font size=^2^ face=^arial^><b> Vehicle No </b></font></td><td align=center style=^width: 10%;^><font size=^2^ face=^arial^><b> Transporter </b></font></td><td align=center style=^width: 10%;^><font size=^2^ face=^arial^><b> Start </b></font></td>"+
			    		  "<td align=center style=^width: 10%;^ ><font size=^2^ face=^arial^><b> Product</b></font></td>"+
	    		  		  "<td align=center style=^width: 5%;^ ><font size=^2^ face=^arial^><b> Weight<br>(Kgs.) </b></font></td>"+
	    		  		  "<td align=center style=^width: 10%;^ ><font size=^2^ face=^arial^><b> Invoice Number</b></font></td>"+	
			    		  "<td align=center style=^width: 20%;^^><font size=^2^ face=^arial^><b> Origin </b></font></td><td align=center style=^width: 25%;^><font size=^2^ face=^arial^><b> Destination </b></font></td> "+
			    		  "<td align=center style=^width: 10%;^><font size=^2^ face=^arial^><b> ETA </b></font></td>"+
			    		  "<td align=center style=^width: 10%;^><font size=^2^ face=^arial^><b> Report On </b></font></td>"+
			    		  "<td align=center style=^width: 10%;^ ><font size=^2^ face=^arial^><b>TripCategory</b></font></td>"+
			    		  "<td align=center style=^width: 15%;^ ><font size=^2^ face=^arial^><b>Vendor</b></font></td>"+
			    		  "<td align=center style=^width: 10%;^ ><font size=^2^ face=^arial^><b>Driver</b></font></td>"+
			    		  "<td align=center style=^width: 5%;^ ><font size=^2^ face=^arial^><b>ContainerNO</b></font></td>"+
			     ...
			     */		  
			    		  ...
			    		  strhtml = strhtml+"<tr><td align=left><font size=^2^ face=^arial^>1</font></td>"+
			    				  ...
			    				  "<td align=left><font size=^2^ face=^arial^>"+tripid+"</font></td>"+
			    ...
			    /* strhtml = strhtml+"<tr><td align=left><font size=^2^ face=^arial^>1</font></td><td align=left><font size=^2^ face=^arial^>"+tripid+"</font></td><td align=left><font size=^2^ face=^arial^>"+veh+"</font></td><td align=left><font size=^2^ face=^arial^>"+owner+"</font></td>"+
			    		  "<td align=left><font size=^2^ face=^arial^>"+calender+" "+sttimehour+":"+sttimemin+"</font></td>"+
			    		  "<td align=left><font size=^2^ face=^arial^>"+GoodsContent+"</font></td>"+
	    		  		  "<td align=left><font size=^2^ face=^arial^>"+weight+"</font></td>"+
	    		  		  "<td align=left><font size=^2^ face=^arial^>"+cnno+"</font></td>"+
 ...
 response.sendRedirect("currentposition.jsp?inserted=yes");
 return; 
}
}
}catch(Exception e)
