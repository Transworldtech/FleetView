...
<!-- code start here -->
		...
		int OScount=0, RAcount=0, RDcount=0,Distance=0,vid=0,j=1;
		double Rating=0.00;
...
%>
			...
			<div align="right">
			<a href="#" onclick="javascript:window.open('print_trip_record1.jsp?data1=<%=data1%>&data2=<%=data2%>&transporter=<%=transporter%>&drivercode=<%=drivercode%>&DriverName=<%=DriverName%>');" title="Print Report"><img src="images/print.jpg" width="15px" height="15px"></a>&nbsp; &nbsp; <a href="excel_trip_record1.jsp?data1=<%=data1%>&data2=<%=data2%>&transporter=<%=transporter%>&drivercode=<%=drivercode%>&name=<%=DriverName%>"  title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a>&nbsp;&nbsp;&nbsp;Date : <%
			...
			<td class="hed">Rating</td>
				...
				double rating1;
				int i=0;
			sql="select * from t_completedjourney WHERE StartDate >= '"+data1+"' AND EndDate <= '"+data2+"' AND (OwnerName LIKE '"+session.getAttribute("usertypevalue").toString()+"' OR GPName LIKE '"+session.getAttribute("usertypevalue").toString()+"')and OwnerName = '"+transporter+"'";
