...
<tr>
<td align="center"><font size="3" color="blue"><b> Detail Report for Vehicle <%=vehregno%> from <%=datenew1 %> <%=fromtime%> to <%=datenew2%> <%=totime%></b></font>
...
<div class="bodyText" align="right">
		...
		<a href="excel_detail_custom_report_bk.jsp?vehcode=<%=VehicleCode%>&data=<%=fromdate%>&data1=<%=todate%>&fromtime=<%=fromtime%>&totime=<%=totime%>&interval=<%=interval1%>&chbx=<%=chbxAll %>&chbx1<%=chbxAll1 %>" title="Export to Excel">
...
<tr>
<br>
<td align="right"> <a  href="temperaturegraph.jsp?vehcode=<%=VehicleCode %>&data=<%=datenew1 %>&data1=<%=datenew2 %>&ftime1=<%=ftime1 %>&ftime2=<%=ftime2 %>&ttime1=<%=ttime1 %>&ttime2=<%=ttime2 %>"><font size="2" face="Arial">Temperature Graph</font></a></td>
</tr>
<%
}
%>
...
<table width="50%" align="center" class="sortable" border="1">
<tr>
<th>Sr.</th>
...
String sqlnew="select * from db_gpsbackup2012.t_veh"+VehicleCode+" where TheFieldDataDateTime <='"+fromdate+" "+fromtime+"' and TheFiledTextFileName in ('SI','OF','ON','ST','SP') order by TheFieldDataDateTime desc limit 1";
...
sql="select * from db_gpsbackup2012.t_veh"+VehicleCode+" where TheFieldDataDateTime >='"+fromdate+" "+fromtime+"' and TheFieldDataDateTime <='"+todate+" "+totime+"' and TheFiledTextFileName in ('SI','OF','ON','ST','SP') order by TheFieldDataDateTime asc";
