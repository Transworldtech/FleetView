...
<%!
Connection con1;
Statement st,st1,st2,st3,st4,st5,st15,st16,st12,st14;
...
<%
   ...
   if(no==0)
   {
	   ...
	   out.println("location='Djettingpump.jsp?VehId="+VehId+"&VehRegNo="+VehRegNo+"'");
	  	out.println("</script>");
...
String abc2="select field from db_gps.t_jettingpumpforcustomfieldmaster where Owner='"+transporter+"' ";
...
int j=0;
int i=1;
String abc="select * from db_gps.t_jettingpumpforcustomfielddata where Field='"+xyz1+"' and Owner='"+transporter+"'";
...
if(rst2.next())
{
	out.println("<script type='text/javascript'>alert('NOT Inserted');");
	 out.println("location='EditDJetting.jsp?VehId="+VehId+"&VehRegNo="+VehRegNo+"'");
 	out.println("</script>");
}
else{
 	count++;
...
//out.println("fieldname--------"+fieldname[0]);
 ...
 String insertString="insert into  db_gps.t_jettingpumpforcustomfielddata(VehRegNo,VehId,Owner,EntryBy,"+finalfielsname+") values('"+VehRegNo+"','"+VehId+"','"+transporter+"','"+user+"',"+finalcolname+") ";
 //out.println("insertString--------"+insertString);
 st15.executeUpdate(insertString);

 
 String insertString1="insert into  db_gps.t_jettingpumpforcustomfieldhistory(VehRegNo,VehId,Owner,EntryBy,"+finalfielsname+") values('"+VehRegNo+"','"+VehId+"','"+transporter+"','"+user+"',"+finalcolname+") ";
 //out.println("insertString--------"+insertString);
 st14.executeUpdate(insertString1);
