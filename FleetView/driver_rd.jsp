...
{  
          ...
          document.RD.action ="excel.jsp";
          document.forms["RD"].submit();
} 

</script>
<%
String exportFileName=session.getAttribute("user").toString()+"_RDExcepReport.xls";
%>
<form id="RD" name="RD"  method="post" >
<div id="table1">
...
<font size="3">Rapid Deceleration Details for Driver <%=dname%> ( <%=dcode%> ), Vehicle <%=vehreg%> of Transporter <%=transporter%> for <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(data1))%>.</font>
...
try{
	...
	sql="select * from t_driver_rd where Transporter='"+transporter+"' and TheDate='"+data1+"' and dcode='"+dcode+"' order by concat(TheDate,TheTime) asc";
