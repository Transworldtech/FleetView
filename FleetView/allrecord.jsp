...
<!-- code start here -->
		...
		connnew = DriverManager.getConnection(MM_dbConn_STRING,MM_dbConn_USERNAME,MM_dbConn_PASSWORD);
			...
			//System.out.println("transporter=="+DriverName);		
%>
			<table class="stats">
			<tr>
			<th colspan="13" >
			...
			<div align="right">
			<a href="#" onclick="javascript:window.open('print_allrecord.jsp?data1=<%=data1%>&data2=<%=data2%>&transporter=<%=transporter%>');" title="Print Report"><img src="images/print.jpg" width="15px" height="15px"></a>&nbsp; &nbsp; <a href="excel_allrecord.jsp?data1=<%=data1%>&data2=<%=data2%>&transporter=<%=transporter%>"  title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a>&nbsp;&nbsp;&nbsp;Date : <%
			...
			<td class="hed">Rating</td>
				...
				double rating1;
				int i=0;
			sql="select RACount,OwnerName,NDCount ,TripID,CRCount,DriverName,DriverID,TripID, RDCount, OSDuration, KmTravelled,EndPlace,StartPlace,EndDate,StartDate,VehId,VehRegNo from t_completedjourney WHERE StartDate >= '"+data1+"' AND EndDate <= '"+data2+"' AND (OwnerName LIKE '"+session.getAttribute("usertypevalue").toString()+"' OR GPName LIKE '"+session.getAttribute("usertypevalue").toString()+"')and OwnerName = '"+transporter+"'";
