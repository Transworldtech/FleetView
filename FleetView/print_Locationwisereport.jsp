				...
				try {
					if (LocationType.equals("Customer")) {
						sqlstop = "SELECT min( duration ) AS Min, max( duration ) AS Max, "
								+ "	count( tripid ) AS Trips, SEC_TO_TIME( SUM( TIME_TO_SEC( duration ) ) / count( * ) ) AS Avg,Location,LocationType  "
								+ "	FROM t_RoundTripStop WHERE LocationType='Customer' and fromdate >= '"
								+ fromdate
								+ " 00:00:00' AND todate <= '"
								+ todate
								+ " 23:59:59' group by Location";
					}

					else if (LocationType.equals("Plant")) {
						sqlstop = "SELECT min( duration ) AS Min, max( duration ) AS Max, "
								+ "	count( tripid ) AS Trips, SEC_TO_TIME( SUM( TIME_TO_SEC( duration ) ) / count( * ) ) AS Avg,Location,LocationType  "
								...
								+ todate
								+ " 23:59:59' group by Location";
					} else if (LocationType.equals("Tollnaka")) {
						sqlstop = "SELECT min( duration ) AS Min, max( duration ) AS Max, "
								+ "	count( tripid ) AS Trips, SEC_TO_TIME( SUM( TIME_TO_SEC( duration ) ) / count( * ) ) AS Avg,Location,LocationType  "
