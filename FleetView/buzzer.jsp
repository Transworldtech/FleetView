...
%>	
 <table width="100%" align="center" border="0"  class="bodytext">
	<tr>
		...
		<font color="block" size="3" ><b>Crane Overload Event Report</font></td>
		</tr>
		<tr><td colspan="8">
		 ...
		 {
		 	var date1=document.getElementById("datan1").value;
			var date2=document.getElementById("datan2").value;
			//alert("Fromdate"+date1);
			//alert("TOdate"+date2);
			...
			<td align="right">
			  ...
			  <input type="hidden" name="datan1" id="datan1" value="<%=fromdate %>">
  			</td>
  			<td align="left">
			...
			<td align="right">
			  	...
			  	<input type="hidden" name="datan2" id="datan2" value="<%=todate %>">
...
<table  border="0" width="100%">
	<tr><td colspan="5" align="center" bgcolor="">
		<font size="3" color="maroon"><b>Crane No <%=cranemodno%> Overload Alarm Report from <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(fromdate))%> to <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(todate))%></font>
 		<div align="right">
         		<a href="#" onclick="javascript:window.print();" title="Print Report">
         		<img src="images/print.jpg" width="15px" height="15px"></a>&nbsp; &nbsp; <a href="Excelbuzzer.jsp?vehcode=<%=vehcode%>&cranemodno=<%=cranemodno %>&fromdate=<%=fromdate%>&todate=<%=todate%>&vehregno=<%=vehregno%>" title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a>&nbsp;&nbsp;&nbsp;
...
<%
	...
	boolean flag = false;
		int i=1;
		String sql="select * from t_veh"+vehcode+" where TheFiledTextFileName in('OE','OD') and TheFieldDataDate >='"+fromdate+" 00:00:00' and TheFieldDataDate <='"+todate+" 23:59:59' order by concat(TheFieldDataDate,TheFieldDataTime) asc ";
		//out.print(sql);
