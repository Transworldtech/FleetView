...
categoryname,startplacename,endplacename,statusname,vehino,vehicleno,vehiclecode,Sqlv="",distributorname="";
 ...
 String searchstatus1="Set",tds="",DriverName="";
%><%
...
<th>Trip Rating</th>
<th>Closed By</th>
<th>Customer Name</th><th>Briefing</th><th>Debriefing</th>
</tr>
<%
...
<td style="text-align: right"><%=rstcloser.getInt("dvcount")%></td>

<td style="text-align: right">

<%
String date31="",date41="";
String vid=rstcloser.getString("Vehid");
int cdcnt=0;
try
{
	date31=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rstcloser.getString("StartDate")));
	date41=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rstcloser.getString("EndDate")));
}
catch(Exception e)
{
	date31=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rstcloser.getString("StartDate")+" "+rstcloser.getString("StartTime")));
	date41=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rstcloser.getString("EndDate")+" "+rstcloser.getString("EndTime")));
}



String sqlcd="select count(*) cdcount from db_gpsExceptions.t_veh"+vid+"_cr where concat(FromDate,' ',FromTime) between '"+date31+"' and '"+date41+"'";
//System.out.println("============sqlcd is==============="+sqlcd);
...
while(rscd.next())
{
	...
	//System.out.println("------cdcnt-----is-----"+cdcnt);
...
My Trips from <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(fromdate))%> to <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(todate))%></B>
	...
	<th class="hed" id="cncol" valign="top">CN</th>
	...
	<th class="hed" id="srcol" valign="top">Customer</th>
	...
	while(rsttrips.next())
	{
		...
		String today111=new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new java.util.Date());
...
%>
	<td><%=rsttrips.getString("cnnumber")%></td>
...
else if(statusname.equalsIgnoreCase("Vehicle Wise Trips") || statusname.equalsIgnoreCase("Driver Wise Trips") )
{
	...
	if(DriverName.equalsIgnoreCase("All"))
	{
	...
	Sqlv="select distinct(tripid),Vehid,TripRating,VehRegNo,OwnerName,StartPlace,StartCode,StartDate,StartTime,EndPlace,EndCode,EndDate,EndTime,StartKm,EndKm,KmTravelled,UnloadDateTime,LoadingDelay,UnloadingDelay,JourneyStatus,DriverID,DriverName,JDCode,Weight,Vendor,Unloadingcomments,DelayReason,OSCount,RACount,RDCount,OSDuration,NDCount,DisconnectionCount from db_gps.t_completedjourney where  (OwnerName='"+session.getAttribute("usertypevalue").toString()+"' OR GPName='"+session.getAttribute("usertypevalue").toString()+"')    and  StartDate BETWEEN '"+fromdate+" 00:00:00' AND '"+todate+" 23:59:59' and DriverName='"+DriverName+"'  "+sum+"  ORDER BY DriverName ";
	}
	}else{
   Sqlv="select distinct(tripid),Vehid,TripRating,VehRegNo,OwnerName,StartPlace,StartCode,StartDate,StartTime,EndPlace,EndCode,EndDate,EndTime,StartKm,EndKm,KmTravelled,UnloadDateTime,LoadingDelay,UnloadingDelay,JourneyStatus,DriverID,DriverName,JDCode,Weight,Vendor,Unloadingcomments,DelayReason,OSCount,RACount,RDCount,OSDuration,NDCount,DisconnectionCount from t_completedjourney where  (OwnerName='"+session.getAttribute("usertypevalue").toString()+"' OR GPName='"+session.getAttribute("usertypevalue").toString()+"')    and  StartDate BETWEEN '"+fromdate+" 00:00:00' AND '"+todate+" 23:59:59' and VehRegNo='"+vehino+"' "+sum+" order by StartDate ";
	}
%>
...
{
	%>
	<font color="brown" size="3">The Trip Report for Driver <%=DriverName%> from <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(fromdate))%> to <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(todate))%></font>
		<%}
else{
	%>
	<font color="brown" size="3">The Trip Report for Vehicle <%=vehino%> from <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(fromdate))%> to <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(todate))%></font>
<%}
		%>
</td>
</tr>
