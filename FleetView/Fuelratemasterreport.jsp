	...
	while(rs.next())
	{
		...
		<tr>
         ...
         <td><div align="left"><%=VehRegNo%></div></td>
			...
			<td><div align="right"><a href="#" onclick="open1('<%=StartDate%>','<%=TripID%>','<%=StartCode%>','<%=EndCode%>','<%=roundOfftotfuel%>','<%=VehRegNo%>','<%=StartPlace%>','<%=EndPlace%>','<%=FIxedKM%>','<%=GPName%>','<%=Customer%>','<%=tripdays%>','<%=Kmpl%>','<%=DriverCode%>','<%=DriverName%>')"><%=TripID%></a></div></td>
