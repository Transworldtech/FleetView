...
<%!
	...
	String fromdate, todate,drivernm,driverlist;
...
try
{
	...
	driverlist=request.getParameter("driverlist");
	String list=driverlist.replace("(,","(");
	//String Transporter=session.getAttribute("usertypevalue").toString();
			...
			if(thename.equals("started"))
			{
				...
				<tr align="center"><td><b><font size="3">Weekly Driving Hrs Violation Report From <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(fromdate)) %> To <%=new SimpleDateFormat("dd-MMM-yyyy").format(new SimpleDateFormat("yyyy-MM-dd").parse(todate)) %></font></b></td></tr>
				<tr><td><div class="bodyText" align="left"> 
	 ...
	 <a href="excel_totalWeeklyhrsViolation.jsp?fromdate=<%=fromdate%>&todate=<%=todate%>&thename=<%="started"%>&driverlist=<%=list %>"  title="Export to Excel"><img src="images/excel.jpg" width="15px" height="15px"></a> 
	 &nbsp;&nbsp;&nbsp;<a href="#" onclick="javascript:window.open('print_totalWeeklyhrsViolation.jsp?fromdate=<%=fromdate%>&todate=<%=todate%>&thename=<%="started"%>&driverlist=<%=list %>');" title="Print Current Position Report"><img src="images/print.jpg" width="15px" height="15px"></a>
...
///************************************************STARTED**********************************************************************************************	
	else if(thename.equalsIgnoreCase("started"))
	{
		...
		<%
				...
				int nd=0;
				String PSvehlist="(";
				String sql="select * from t_CastrolDrivers6 where TheDte between '"+fromdate+"' and '"+todate+"' AND LastWeekRDurInHrs > 60 and driverid IN "+list+" order by TheDte Asc";
