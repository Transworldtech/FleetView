...
<% 
	...
	String filename=session.getAttribute("user").toString()+showdatex+"UnitDetails_Report.xls";
...
fromdatetime=request.getParameter("data");
todatetime=request.getParameter("data1");
...
while(rst.next())
{
	if(rst.getString("TheFiledTextFileName").equals("OF"))
	{
		...
		if(mils1>mils2)
		{
		String sql11="select * from t_veh"+vehcode+" where concat(TheFieldDataDate,' ',TheFieldDataTime) >='"+fdate+" "+ftime+"' and TheFiledTextFileName NOT IN ('OF','VD') order by concat(TheFieldDataDate,' ',TheFieldDataTime) asc";
		...
		if(rst11.next())
		{
			...
			//if((!(rstunit.getString("TheFiledTextFileName").equalsIgnoreCase("VD"))) || (!(rstunit.getString("TheFiledTextFileName").equalsIgnoreCase("OF"))))
				...
				<td class="bodyText"><div align="left"><%=ds2-ds1%></div></td>
				<%
				k++;
						}
					//rst.afterLast();
