...
String sql1="select * from t_vehicledetails where VehicleCode='"+VehicleCode+"'";
	...
	String sql="select * from t_veh"+VehicleCode+" where concat(TheFieldDataDate,TheFieldDataTime)>='"+fromdate+fff+"' and concat(TheFieldDataDate,TheFieldDataTime)<='"+todate+ttt+"' and TheFiledTextFileName in('SI','DO','DS') order by concat(TheFieldDataDate,TheFieldDataTime) asc";
rst1=st1.executeQuery(sql);
%>

<center><font size="2" class="bodyText"><b>The Detail Report For the Vehicle <%=session.getAttribute("hvrno").toString()%> For the Period :<%=session.getAttribute("HisShowDate").toString() %> <%=fff%> to <%=session.getAttribute("HisShowDate1").toString() %> <%=ttt%></b></font></center><div class="bodyText" align="right">

<a href="#" onclick="javascript:window.open('printtempraturereport.jsp');" title="Print Detail Report">
<img src="images/print.jpg" width="15px" height="15px">
</a>&nbsp; &nbsp; 
<a href="exceltempraturereport.jsp"  title="Export to Excel">
...
<tr ><td class="bodyText" style="text-align: left"><% if(rst1.getString("TheFiledTExtFileName").equals("DS")) out.print("<font color='green'>Door Close  </font>");%><% if(rst1.getString("TheFiledTExtFileName").equals("DO")) out.print("<font color='red'>Door Open  </font>");%>
...
%>
<%=rst1.getString("TheFieldDataTime")%></td><td class="bodyText" style="text-align: left"><%=rst1.getString("Speed")%></td><td class="bodyText" style="text-align: right"><%=Integer.parseInt(rst1.getString("Distance"))-dist%></td><td class="bodyText"><div align="left"><% String loc=rst1.getString("TheFieldSubject"); if(loc.equals("-")) out.print("No Data");
else 
{
...
%>
<!-- <td class="bodyText"><% if(rst1.getString("Sen1").equals("-")){ out.print("-");} else out.print((Integer.parseInt(rst1.getString("Sen1"))-fuel)*0.005);%></td> --> 
...
%>
<%=rst1.getString("TheFieldDataTime")%></td><td class="bodyText" style="text-align: left"><%="STOP"%></td><td class="bodyText" style="text-align: right"><%=Integer.parseInt(rst1.getString("Distance"))-dist%></td><td class="bodyText"><div align="left"><% String loc=rst1.getString("TheFieldSubject"); if(loc.equals("-")) out.print("No Data");
else 
{
if(session.getAttribute("usertypevalue").toString().equals("Om"))
{
